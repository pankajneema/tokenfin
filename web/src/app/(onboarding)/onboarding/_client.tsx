  ...
  async function handleProjectNext(projectName: string, projectSlug: string, projectDesc: string) {
    ...
    try {
      const res = await fetch('/api/v1/projects', {
        ...
        body:    JSON.stringify({ org_id: orgId, name: projectName, slug: projectSlug, description: projectDesc }),
      })
      if (!res.ok) throw new Error(await res.text())
      const project = await res.json()
      patch({ projectName, projectSlug, projectDesc, projectId: project.id, orgId })
      setStep(1)
    } catch (e: any) {
      setError(e.message)
    } finally {
          ...
          {step < 2 && (
            <div className="flex flex-col items-center mb-10">
              ...
              <div className="mt-6 text-center">
                <p className="text-[11px] font-semibold tracking-widest text-[var(--fg-tertiary)] uppercase mb-1.5">
                  Step {step + 1} of 2
                </p>
