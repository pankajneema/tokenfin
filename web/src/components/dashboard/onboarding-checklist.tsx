...
const STORAGE_KEY = 'tf_onboarding_dismissed_v1'
      ...
      >
        ...
        <svg width="36" height="36" viewBox="0 0 36 36" className="flex-shrink-0 -rotate-90">
          ...
          <text
            ...
            fontSize="9" fontWeight="700" fill="var(--fg)"
        ...
        <div className="border-t border-[var(--border)] divide-y divide-[var(--border)]">
          {steps.map((step, i) => (
            <Link
              key={step.id}
              href={step.href}
              onClick={step.id === 'event' ? dismiss : undefined}
              className={cn(
                'flex items-start gap-3 px-5 py-3.5 transition-colors',
                step.done
                  ? 'opacity-50 pointer-events-none'
                  : step.current
                    ? 'bg-[var(--green-bg)] hover:brightness-95'
            ...
            >
              <div className="mt-0.5 flex-shrink-0">
                {step.done
                  ...
                  : <Circle size={17} className={step.current ? 'text-[var(--green)]' : 'text-[var(--fg-tertiary)]'} />
                }
              </div>
