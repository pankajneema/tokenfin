...
| `/api/otel/v1/metrics` | POST | API key (Bearer/`?key=`) | OTLP receiver — health checks + **Codex/Gemini capture**: derives per-turn rows by diffing cumulative counters (`lib/otlp/metrics.ts`, state in `otlp_metric_state`, migration 024). Codex metrics are NOT derived (its logs own those rows). |
...
- `/dashboard/setup` — interim Codex connect surface (OTLP one-command + live ingest beacon). Full grouped `/connections` is a later milestone.
...
`~/.Codex/settings.json` pointing Codex at `/api/otel`.
...
- **Per-turn rows come from logs**: Codex's `Codex.api_request` event carries model,
  ...
  Metrics (`Codex.token.usage`) are cumulative counters of the same tokens → health-only,
